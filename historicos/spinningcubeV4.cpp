...
int main(int argc, char** argv) {
    ...
    // Load the texture image for the red cube
    osg::ref_ptr<osg::Image> redTexture = osgDB::readImageFile("red.rgb");
    if (redTexture) {
        // Create a texture object and set the loaded image
        osg::ref_ptr<osg::Texture2D> texture = new osg::Texture2D;
        texture->setImage(redTexture);
        ...
        osg::notify(osg::WARN) << "Error loading red.rgb" << std::endl;
    }
    shapeDrawable->setColor(osg::Vec4(1.0f, 1.0f, 1.0f, 1.0f)); //color
    ...
    osg::ref_ptr<osg::Image> blueTexture = osgDB::readImageFile("blue.rgb");
    if (blueTexture) {
        // Create a texture object and set the loaded image
        osg::ref_ptr<osg::Texture2D> texture2 = new osg::Texture2D;
        texture2->setImage(blueTexture);
        ...
        // Handle error if texture loading fails
        osg::notify(osg::WARN) << "Error loading blue.rgb" << std::endl;
    }
    shapeDrawable2->setColor(osg::Vec4(1.0f, 1.0f, 1.0f, 1.0f)); //color
